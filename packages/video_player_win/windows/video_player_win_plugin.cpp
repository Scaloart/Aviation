...
public:
  ScreenOnKeeper() {
    ...
    if (g_handle == 0) {
      ...
      ctx.Reason.SimpleReasonString = (PWSTR)L"[video_player_win]";
  ...
  if (isOpenVideo) {
    ...
    HRESULT hr = player->OpenURL(wPath, player, hwnd, [=](bool isSuccess) {
      if (isSuccess) {
        ...
        map[flutter::EncodableValue("videoWidth")] = flutter::EncodableValue(static_cast<int32_t>(videoSize.cx));
        map[flutter::EncodableValue("videoHeight")] = flutter::EncodableValue(static_cast<int32_t>(videoSize.cy));
    ...
    int64_t ms = static_cast<int64_t>(player->GetCurrentPosition());
    result->Success(flutter::EncodableValue(ms));
  } else if (method_call.method_name().compare("getDuration") == 0) {
    int64_t ms = static_cast<int64_t>(player->GetDuration());
    result->Success(flutter::EncodableValue(ms));
